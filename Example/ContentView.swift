...
struct ContentView: View {
    ...
    var body: some View {
        NavigationView {
            ScrollView {
                VStack(spacing: 16) {
                    if let braze = analyticsManager.braze {
                        BannerVerificationView(braze: braze)
                    }
...
private struct BannerVerificationView: View {
    let braze: Braze
    ...
    private func bannerView(placement: String) -> BrazeBannerUI.BannerView {
        var view = BrazeBannerUI.BannerView(placementId: placement, braze: braze)
        view.onDismiss = { event in
            banner = nil
